...
void sm3_compress(u4 digest[8], const u1* block, size_t nblocks)
{
	...
	for (i = 0; i < nblocks; i++)
	{
		...
		{
			SS2 = W[j - 3];
			SS1 = W[j - 16] ^ W[j - 9] ^ ROTL32(SS2, 15);
			SS2 = W[j - 13];
			W[j] = P1(SS1) ^ ROTL32(SS2, 7) ^ W[j - 6];
		}
...
#define IPAD	0x36
#define OPAD	0x5C
void sm3_hmac_init(SM3_HMAC_CTX *ctx, const u1 *key, size_t keylen)
{
	uint32_t i = 0;
